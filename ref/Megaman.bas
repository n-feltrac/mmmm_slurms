...
''  Megaman Style Controls
''
''	Relminator (Richard Eric M. Lope)
''	http://rel.phatcode.net
''
''	Entity management using Finite State Machines(FSM)
''	FSM is a programming paradigm where there can only
''	be 1 state for an entity in a given time.
''
''	FSMs make entity management easier because it is easy to
''	visualize logic by limiting action to a single state
''	in a given frame(or time).
''
''	Code may be a little bigger but the consequences of
''	not using it( if FSMs is suitable ) is very messy 
...
Sub Player.UpdateFiring()
    ...
    if( not multikey(FB.SC_Z) ) then FireCounter += 1
...
sub Player.Draw(SpriteSet() as GL2D.IMAGE ptr)
	...
	End Select
		...
		GL2D.SpriteFlip( x + FrameXoff, y + FrameYoff, FlipMode, SpriteSet(BaseFrame + Frame))
...
sub main()
	...
	GL2D.InitSprites( MegamanImages(), megaman_sprites_texcoords(), "megaman_sprites.bmp", GL_NEAREST )
	...
	do
		...
		GL2D.Begin2D()
			...
			GL2D.Line2D( LADDER_VALUE, 0 , LADDER_VALUE, FLOOR_VALUE, GL2D_RGBA(0,255,0,255) )
			dim as Integer Steps = FLOOR_VALUE \ 16
			for i as Integer = 0 to Steps
				GL2D.Line2D( LADDER_VALUE - 16, i * 16 , LADDER_VALUE + 16, i * 16, GL2D_RGBA(0,255,0,255) )
			Next
			'' Draw Player
			Mega.Draw(MegamanImages())
			
			GL2D.PrintScale(0,  140, 1, "Entity management using FSM")    
			GL2D.PrintScale(0,  150, 1, "Code by Relminator")    
